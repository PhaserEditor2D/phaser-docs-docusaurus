---
slug: script-node-class
title: The ScriptNode class
---

When you create an instance of a ScriptNode, the [scene compiler](../scene-compiler) generates a code like this:
...
The good news is that this class is included in the Phaser Editor project templates. If your project is not based on a template, you can install it from the `@phaserjs/editor-scripts-core` script library. [Learn more about the script libraries](script-node-libraries).

Also, Phaser Editor can generate the default implementation of this class for you, with the basic features:

1. In the [Files view](../../workbench/files-view), select the folder when you want to add the class file.
2. Open the [Command Palette](../../workbench/command-palette) (`Ctrl+K`) and search for `script`.
...
![Create ScriptNode class commands.](https://cdn.phaser.io/editor-docs/script-node-create-script-node-class-1-20230323.webp)
...
* Registers listeners to the scene and game object for implementing the **awake**, **start**, **update**, and **destroy** events. It follows the same logic as the [User Components events](../user-components/user-components-super-class).
...
The `ScriptNode` class provides a couple of methods for handling special events that may help you implement the behaviors. It works just like the [User Components events](../user-components/user-components-super-class). The methods are:

- `awake()`: It is called when all the objects of the scene are created. The values of the user properties (prefab) will be available at this time, so you can override this method for making computations that require the value of the properties. It works like the [UserComponent “awake” method](../user-components/user-components-awake-event).
...
![The scene for collecting coins](https://cdn.phaser.io/editor-docs/scene-editor-script-node-target-action-game-20240109.webp)
...
![Make collider node.](https://cdn.phaser.io/editor-docs/scene-editor-script-node-demo-collider-node-20240109.webp)![Make collider node properties](https://cdn.phaser.io/editor-docs/scene-editor-script-node-demo-collider-node-props-20240109.webp)
...
![The destroy action node](https://cdn.phaser.io/editor-docs/scene-editor-script-node-collider-demo-destroy-action-20240109.webp)![The properties of the destroy action](https://cdn.phaser.io/editor-docs/scene-editor-script-node-collider-demo-destroy-action-properties-20240109.webp)
